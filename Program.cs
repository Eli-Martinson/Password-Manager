...
//      3. Store passwords in a dictionary with associated usernames--DONE
//      4. Login  system integration and final testing--DONE
...
public class PasswordGenerator
{
    ...
    {
        //prompts user with an option to sign up or log in and continues based on choice
        Console.WriteLine("Welcome! Would you like to (1) Sign up or (2) Log in?");
        ...
        {
            case "1":
                SecurityManager.CreateMasterLogin();
                Console.WriteLine("Please restart the application to log in.");
                return; // Exit after sign-up
            case "2":
                verified = SecurityManager.VerifyMasterPassword();
                break;
            default:
...
public static class SecurityManager
{
    ...
    public static void CreateMasterLogin() //creates new login
    {
        string loginFile = "master.login";
        ...
        File.WriteAllText(loginFile, $"{newUsername}:{hashedPassword}");
        Console.WriteLine("Master credentials set.");
    }

    // Verifies the master login info by comparing hashed input with stored hash
    public static bool VerifyMasterPassword()
    {
        string loginFile = "master.login";

        if (!File.Exists(loginFile)) // First time setup
        {
            Console.WriteLine("No account found. Please sign up first.");
            return false;
        }
        Console.Write("Enter username: ");
        string inputUsername = Console.ReadLine();
        string hashedInputUsername = Hash(inputUsername);

        Console.Write("Enter master password: ");
        string inputPassword = Console.ReadLine();
        string hashedInputPassword = Hash(inputPassword);

        string[] storedLogin = File.ReadAllText(loginFile).Split(':');
        string storedUsername = storedLogin[0];
        string storedPassword = storedLogin[1];

        return inputUsername == storedUsername && hashedInputPassword == storedPassword;
    }
}
